...
cosfire_symm = c.COSFIRE(
		c.CircleStrategy, c.DoGFilter, (1.8, 1), rhoList=range(0,9,2), sigma0=3/6,  alpha=0.7/6
	   ).fit(proto_symm, (cx, cy))
print(cosfire_symm.strategy.tuples)
result_symm = cosfire_symm.transform(subject)

cosfire_asymm = c.COSFIRE(
		c.CircleStrategy, c.DoGFilter, (2.4, 1), rhoList=range(0,23,2), sigma0=2/6,  alpha=0.1/6
