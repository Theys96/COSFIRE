...
class CircleStrategy(BaseEstimator, TransformerMixin):
	...
	def __init__(self, filt, filterArgs, rhoList, prototype, center, sigma0=0, alpha=0, rotationInvariance=[0], scaleInvariance=[1], T1=0, T2=0, grid=(1,1)):
	...
	def transform(self, subject):
		...
		(gridX, gridY) = self.grid
		(h,w) = subject.shape
		m = self.maxRho()
		#print("{} x {}".format(w, h))
		tasks = {}
		pool = Pool(gridX*gridY)
		print("grid: {},{}".format(gridX, gridY))
		for x in range(gridX):
			for y in range(gridY):
				lowerX = max(0, x * int(w/gridX))
				upperX = min(w, (x+(x+1==gridX)+1) * int(w/gridX))
				lowerY = max(0, y * int(h/gridY))
				upperY = min(h, (y+(y+1==gridY)+1) * int(h/gridY))
				marginLowerX = min(lowerX, m)
				marginUpperX = min(w-upperX, m)
				marginLowerY = min(lowerY, m)
				marginUpperY = min(h-upperY, m)
				chunk = subject[(lowerY - marginLowerY):(upperY + marginUpperY),(lowerX - marginLowerX):(upperX + marginUpperX)]
				tasks[x,y] = pool.apply_async(apply, (chunk, self.tuples, self.filt, self.sigma0, self.alpha, self.rotationInvariance, self.scaleInvariance) )

		result = np.empty(subject.shape)
		for x in range(gridX):
			for y in range(gridY):
				lowerX = max(0, x * int(w/gridX))
				upperX = min(w, (x+(x+1==gridX)+1) * int(w/gridX))
				...
				marginLowerY = min(lowerY, m)
				marginUpperY = min(h-upperY, m)
				chunk = tasks[x,y].get()[marginLowerY:(marginLowerY+upperY-lowerY),marginLowerX:(marginLowerX+upperX-lowerX)]
				result[lowerY:upperY,lowerX:upperX] = chunk
